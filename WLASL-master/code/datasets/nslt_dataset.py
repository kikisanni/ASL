...
def load_rgb_frames(image_dir, vid, start, num):
    ...
    for i in range(start, start + num):
        ...
        if img.ndim == 3:
            ...
            h, w = img.shape
        c = 1  # Grayscale image, so only one channel
...
def load_rgb_frames_from_video(vid_root, vid, start, num, resize=(256, 256)):
    ...
    for offset in range(min(num, int(total_frames - start))):
        ...
        if not success or img is None:
            print(f"Failed to read frame at {offset} from {video_path}")
            continue  # Skip this frame or handle the failure appropriately

        if img.ndim == 3:
            h, w, c = img.shape
        elif img.ndim == 2:
            h, w = img.shape
            c = 1  # Assuming grayscale, so only one channel
